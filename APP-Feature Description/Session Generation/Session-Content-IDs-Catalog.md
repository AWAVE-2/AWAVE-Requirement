...
- **Umsetzung (Stand 2026-02):** In `SessionContentMapping` ist die Map **`voiceThemeToSoundIdVariants`** für die drei Kategorie-Themes (Einschlafen, Innere_Ruhe, Motivation) mit je 5 Dokument-IDs pro Stimme hinterlegt; `soundIdFromVoiceTopic` wertet das Suffix `/vN` aus und liefert pro Slot eine andere Firestore-Dokument-ID. Damit erhalten die 5 Sessions pro Kategorie unterschiedliche Text-Tracks (keine Duplikate). Doku: `docs/handovers/HANDOVER_SESSION_TEXT_VARIANTEN_KEINE_DUPLIKATE.md`, `docs/ABSCHLUSSBERICHT-SESSION-TEXT-VARIANTEN.md`.
